function [fig1, fig2, fig3, fig4] = plot2(Name,pop,cases,deaths,recovered,plotType,plotFlag)
%--------------------------------------------------------------------------
% plot2.m
% 
...
fprintf('%20s*| cases: %5s percent | rate: %5s | accel: %5s | death: %5s | deathRate: %5s \n',...
    Name,...
    num2str((100*cases(end)./pop),3),...
    num2str(floor(Dcases(end)),6),...
    num2str(floor(DDcases(end)),6),...
    num2str(floor(deaths(end)),6),...
    num2str(floor(Ddeaths(end)),6)...
    );
